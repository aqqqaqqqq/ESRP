...
import omnigibson.robots
import omnigibson.robots.manipulation_robot
import torch as th
...
class ManipulationRobot(BaseRobot):
    ...
    def _calculate_in_hand_object_rigid(self, arm="default"):
        ...
        """
        assert isinstance(self, omnigibson.robots.Test), 'This class is tailored for the Test Robot'
        THETA = math.pi / 2
        R = 1.0
        VISIBLE_AREA_R = R * 2
        arm = self.default_arm if arm == "default" else arm
        ...
        bbox_in_world = []
        polygons = []
        for _obj in list(self.scene._init_objs.values()):
        ...
        yaw = self.get_yaw()
        robot_x, _, robot_z = self.get_position_orientation()[0]
        ...
        if len(sorted_polygons):
            ...
            ag_obj = self.scene.object_registry("prim_path", ag_obj_prim_path) # type: ignore
    ...
    def _establish_grasp_rigid(self, arm="default", ag_data=None, contact_pos=None):
        ...
        # joint_frame_pos = th.tensor([0, 0, 0, 1.0])
        joint_frame_pos = th.tensor([0, 0, 0])
        joint_frame_orn = th.tensor([0, 0, 0, 1.0])
        eef_link_pos, eef_link_orn = self.links['base_link'].get_position_orientation()
        ...
        # Create the joint
        joint_prim_path = f"{self.links['base_link'].prim_path}/ag_constraint"
        # import pdb; pdb.set_trace()
        joint_prim = create_joint(
            prim_path=joint_prim_path,
            joint_type='FixedJoint',
            body0=self.links['base_link'].prim_path,
    ...
    def _establish_grasp_cloth(self, arm="default", ag_data=None):
        ...
        # Modify max force based on user-determined assist parameters
        # TODO
        max_force = (m.MIN_ASSIST_FORCE + (m.MAX_ASSIST_FORCE - m.MIN_ASSIST_FORCE) * m.ASSIST_FRACTION) * 5
        # max_force = 1e10
        # joint_prim.GetAttribute("physics:breakForce").Set(max_force)

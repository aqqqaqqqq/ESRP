...
def log_episode_info(episode, **kwargs):
    if len(logger._core.handlers) <= 1:
        logger.add(f"/home/pilab/Desktop/wq/rearrange/rllib/rllib_{datetime.now().strftime('%Y%m%d-%H%M%S')}_callback.log", encoding="utf-8")
    all_things = {}
    ...
    logger.info(json.dumps(all_things, default=tensor_to_serializable))
...
if __name__ == "__main__":
    ...
    base_config = (
        ...
        .env_runners(sample_timeout_s=6000.0, num_gpus_per_env_runner=0.33, num_env_runners=6, create_env_on_local_worker=True)
    ...
    # logger.add(f"/home/pilab/Desktop/wq/rearrange/rllib/rllib_{datetime.now().strftime('%Y%m%d-%H%M%S')}.log", encoding="utf-8")
    step = 0
    while True:
        result = algo.train()
        ...
        if step % NUM_UPDATES_PER_SAVING == 0:
            module.save_to_path(os.path.join('saved_models/', str(step)))
        step += 1

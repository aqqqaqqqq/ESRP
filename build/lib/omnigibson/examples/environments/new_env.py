...
class FastEnv(gym.Env):
    ...
    def step(self, action):
        ...
        observations, rewards, terminates, truncates, infos = no_physical_step(self.env, no_op_action, n_render_iterations=2)

        if self.grasping_obj is not None:
            number = th.tensor(1, dtype=observations.dtype)
            observations = th.cat([observations, number.unsqueeze(0)])
        else:
            number = th.tensor(0, dtype=observations.dtype)
            observations = th.cat([observations, number.unsqueeze(0)])        
...
def make_env(cfg):
    ...
    scene_names = []
    with open('/home/pilab/Siqi/4_28/OmniGibson-Rearrange/omnigibson/data/train_data.txt', 'r') as f:
        for line in f:

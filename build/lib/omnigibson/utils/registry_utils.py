...
class Registry:
    ...
    def remove(self, obj):
        ...
        for k in self.all_keys:
            ...
            # if attribute lookup failed, skip
            if obj_attr == m.DOES_NOT_EXIST:
                continue

            # normalize to list
            if not isinstance(obj_attr, Iterable) or isinstance(obj_attr, str):
                attrs = [obj_attr]
            else:
                attrs = obj_attr
            ...
            mapping = self.get_dict(k)
            if k in self.unique_keys:
                # remove unique mapping safely
                for attr in attrs:
                    mapping.pop(attr, None)
            else:
                # remove from group mapping safely
                for attr in attrs:
                    if attr in mapping:
                        mapping[attr].discard(obj)
                        # if that set is now empty, drop the key entirely
                        if not mapping[attr]:
                            del mapping[attr]

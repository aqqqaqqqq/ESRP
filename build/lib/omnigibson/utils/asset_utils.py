...
def get_available_3dfront_scenes():
    """
    3d front interactive scenes

    Returns:
        list: Available 3d front interactive scenes
    """
    threed_front_path = gm.ThreeD_FRONT_DATASET_PATH
    scenes_path = os.path.join(threed_front_path, "scenes")
    available_3dfront_scenes = sorted([f for f in os.listdir(scenes_path) ])
    return available_3dfront_scenes

def get_available_3dfront_target_scenes(scene):
    """
    3d front interactive scenes
    ...
    """
    threed_front_path = gm.ThreeD_FRONT_DATASET_PATH
    scenes_path = os.path.join(threed_front_path, "scenes", scene)
    available_3dfront_scenes = sorted([f for f in os.listdir(scenes_path) ])
    return available_3dfront_scenes

def get_available_3dfront_rooms(scene_id):
    """
    3d front interactive scenes
    ...
    threed_front_path = gm.ThreeD_FRONT_DATASET_PATH
    scenes_path = os.path.join(threed_front_path, "scenes")
    rooms_path = os.path.join(scenes_path, scene_id)
    available_3dfront_rooms = sorted([f for f in os.listdir(rooms_path) ])
    return available_3dfront_rooms


def get_available_3dfront_room(scene_id, room_type):
    """
    3d front interactive scenes
    ...
    scenes_path = os.path.join(threed_front_path, "scenes")
    rooms_path = os.path.join(scenes_path, scene_id)
    room_path = os.path.join(rooms_path, room_type)
    available_3dfront_room = sorted([f.split('.')[0] for f in os.listdir(room_path) ])
    return available_3dfront_room

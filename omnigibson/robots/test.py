...
# Monkey patch
from omnigibson.robots.monkey_patch_manipulation_robot import ManipulationRobot
ArticulatedTrunkRobot.__bases__ = (ManipulationRobot,)
MobileManipulationRobot.__bases__ = (ManipulationRobot,)

class Test(TwoWheelRobot, ArticulatedTrunkRobot, UntuckedArmPoseRobot):
    """
    Fetch Robot
    ...
    def disabled_collision_pairs(self):
        return [
            ...
            ["torso_lift_link", "virtual_arm_link"],
            ["torso_fixed_link", "virtual_arm_link"],
            ["base_link", "virtual_arm_link"],
            ["l_wheel_link", "virtual_arm_link"],
            ["r_wheel_link", "virtual_arm_link"],
        ]
    ...
    def arm_link_names(self):
        return {
            self.default_arm: ["virtual_arm_link"
            ]
        }
    ...
    def arm_joint_names(self):
        return {
            self.default_arm: ["virtual_arm_joint"
            ]
        }
    ...
    @property
    def usd_path(self):
        return os.path.join(gm.ASSET_PATH, "models/new/fetch/fetch.usd")

    @property
    def robot_arm_descriptor_yamls(self):
        return {self.default_arm: os.path.join(gm.ASSET_PATH, "models/new/fetch_descriptor.yaml")}

    @property
    def urdf_path(self):
        return os.path.join(gm.ASSET_PATH, "models/new/fetch.urdf")
    ...
    def eef_usd_path(self):
        return {self.default_arm: os.path.join(gm.ASSET_PATH, "models/new/fetch/fetch_eef.usd")}
    ...
    def get_yaw(self):
        ...
        robot_pos, robot_quat = self.get_position_orientation()
        x = th.tensor([1.0, 0, 0])
        trans_x = T.quat_apply(robot_quat, x)
        return th.atan2(trans_x[2], trans_x[0])
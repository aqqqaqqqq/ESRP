...
from omnigibson.utils.gym_utils import (
    ...
    recursively_generate_rearrange_dict,
)
...
class Environment(gym.Env, GymObservable, Recreatable):
    ...
    def __init__(self, configs, in_vec_env=False):
        ...
        self._modify_reload_model = self.env_config["modify_reload_model"]

        if self._modify_reload_model:
            threed_front_path = gm.ThreeD_FRONT_DATASET_PATH
            self._scene_dir = os.path.join(threed_front_path, "scenes")
            # self.scene_names = []
            # for entry in os.listdir(self._scene_dir):
            #     entry_path = os.path.join(self._scene_dir, entry)
            #     if os.path.isdir(entry_path):
            #         self.scene_names.append(entry)
            self.scene_names = self.env_config["scene_names"]
            scene_name = random.choice(self.scene_names)
            self._scene_name = scene_name
            scene_model = scene_name + "_initial.json"
            self._layout_path = os.path.join(self._scene_dir, scene_name, scene_name + "_target.png")
            scene_path = os.path.join(self._scene_dir, scene_name)
            self.scene_config["scene_model"] = scene_model
            self.scene_config["scene_type_path"] = scene_path
            self._initial_scene = False
    ...
    def modified_reload_model(self):
        """
        Reload another scene model.
        This allows one to change the scene on the fly.
        """
        if og.sim is not None:
            for _env in og.sim._scenes:
        ...
        scene_model = scene_name + "_initial.json"
        self._layout_path = os.path.join(self._scene_dir, scene_name, scene_name + "_target.png")
        scene_path = os.path.join(gm.ThreeD_FRONT_DATASET_PATH, "scenes", scene_name)
        self.scene_config["scene_model"] = scene_model
        self.scene_config["scene_type_path"] = scene_path
        self.load()
        if og.sim is not None:
    ...
    def _load_scene_rearrange(self):
        ...
        if hasattr(self, 'scene') and self.scene is not None:
            idx = og.sim._scenes.index(self.scene)
        # Create the scene from our scene config
        self._scene = create_class_from_registry_and_config(
            cls_name=self.scene_config["type"],
            cls_registry=REGISTERED_SCENES,
        ...
        og.sim.import_scene_rearrange(self._scene, idx)
        assert og.sim.is_stopped(), "Simulator must be stopped after loading scene!"
    ...
    def load(self):
        ...
        if self._modify_reload_model and self._initial_scene:
            self._load_scene_rearrange()
        else:
            self._load_scene()

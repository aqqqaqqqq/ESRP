...
from omnigibson.utils.model_utils import (
    LSTMContainingRLModule, NoLSTMRLModule
)
...
def main(random_selection=False, headless=False, short_exec=False, quickstart=False):
    spec = RLModuleSpec(NoLSTMRLModule, observation_space=Box(low=0, high=255, shape=(98306,), dtype=np.uint8), action_space = gym.spaces.Discrete(6))
    rl_module = spec.build()
    rl_module.restore_from_path(path = '....../rearrange/model_nolstm/3000')
    ...
    scene_names = []
    content = []
    result_log = "....../rearrange/model_nolstm/nolstm_val.log"
    with open(result_log, 'r') as f:
        content = f.read()  # Entire file content as one string

    with open('....../omnigibson/data/3d_front/train_sampled.txt', 'r') as f:
        for line in f:
    ...
    logger.add(result_log, encoding="utf-8")
    ...
    while len(rearrangement_env.env.scene_names) > 0:
        ...
        while True:
            ...
            _init = {Columns.OBS: obs.unsqueeze(0)}
